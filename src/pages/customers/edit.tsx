...
import { updateCustomer, type Customer } from '../api/api_services/customer';
...
const EditCustomer: React.FC<{
  ...
  onSave: () => void;
  onCancel: () => void;
}> = ({ isDarkTheme, customer, onSave, onCancel }) => {
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <h1 className={`text-3xl font-bold mb-2 ${isDarkTheme ? 'text-white' : 'text-gray-900'}`}>Edit Customer</h1>
        <p className={isDarkTheme ? 'text-slate-400' : 'text-gray-600'}>Update customer information</p>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className={`p-4 rounded-xl border ${isDarkTheme ? 'bg-slate-900 border-slate-700' : 'bg-gray-50 border-gray-200'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {field('full_name', 'Full Name', 'Full name', User)}
              {field('company_name', 'Company Name', 'Company Ltd', Building2, 'text', false)}
            </div>
          </div>
